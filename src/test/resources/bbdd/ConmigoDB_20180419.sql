...
CREATE TABLE `chat` (
  ...
  PRIMARY KEY (`ID`),
  KEY `MESSAGE_IDX` (`ID`),
  KEY `MESSAGE_EVENT_FK` (`EVENT`),
  KEY `MESSAGE_SENDER_FK` (`SENDER`),
  KEY `MESSAGE_RECEIVER_FK` (`RECEIVER`),
  CONSTRAINT `MESSAGE_EVENT_FK` FOREIGN KEY (`EVENT`) REFERENCES `event` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `MESSAGE_RECEIVER_FK` FOREIGN KEY (`RECEIVER`) REFERENCES `user` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  CONSTRAINT `MESSAGE_SENDER_FK` FOREIGN KEY (`SENDER`) REFERENCES `user` (`ID`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=3 DEFAULT CHARSET=utf8 COMMENT='Tabla de Mensajes';
...
-- Dump completed on 2018-04-19 21:13:34
